  ...
  ShouldSpec({
    ...
    should("store uploaded templates under their SHA-256 like Carbone") {
      ...
      fake.calls
        .shouldHaveSize(1)
        .first()
        .shouldBeInstanceOf<FakeKarbone.Call.Upload>()
        .options
    ...
    should("inject one failure with failNextWith, then recover") {
      val fake = FakeKarbone()
      fake.failNextWith(KarboneError.Unauthorized("nope"))
      either { fake.status() }
        .shouldBeInstanceOf<Either.Left<KarboneError>>()
        .value
    ...
    should("use a custom renderer") {
      val fake = FakeKarbone(renderer = FakeKarbone.Renderer { "%PDF-fake".toByteArray() })
      val doc = either {
        fake.renders.render(
          TemplateSource.bytes(content),
          RenderData.Empty,
