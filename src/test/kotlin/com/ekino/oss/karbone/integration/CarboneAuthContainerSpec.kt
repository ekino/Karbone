  ...
  ShouldSpec({
    ...
    context("carbone-ee container with authentication").config(enabled = dockerAvailable) {
      should("answer /status without a token") {
        val status = either { Karbone.onPremise(baseUrl).status() }
        status.shouldBeInstanceOf<Either.Right<*>>()
      }

      should("reject a render without a token as Unauthorized, keeping the server message") {
        val result = either { Karbone.onPremise(baseUrl).renders.render(template, data) }
        val error =
          result
            .shouldBeInstanceOf<Either.Left<KarboneError>>()
            .value
            .shouldBeInstanceOf<KarboneError.Unauthorized>()
        error.status shouldBe 401
      }

      should("reject a forged token as Unauthorized") {
        val result = either {
          Karbone.onPremise(baseUrl, token = "not-a-jwt").renders.render(template, data)
        }
        result
          .shouldBeInstanceOf<Either.Left<KarboneError>>()
          .value
      ...
      should("upload and render with a valid JWT") {
        val karbone = Karbone.onPremise(baseUrl, token = token)
        val doc = either {
        ...
        either { karbone.templates.delete(template.sha256()) }.shouldBeInstanceOf<Either.Right<*>>()
      }
    }
  })
