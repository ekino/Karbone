...
class KarbonStubServerSpec :
  ShouldSpec({
    ...
    should("parse status() from a plain JSON envelope") {
      ...
      val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
      val result = either<KarbonError, ApiStatus> { karbon.status() }
    ...
    context("headers") {
      should(
        "send Authorization: Bearer and no carbone-version on-premise, and a karbon/ User-Agent"
      ) {
        stub.handler = HttpHandler { ex -> ex.respondJson(200, STATUS_BODY) }
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        either<KarbonError, ApiStatus> { karbon.status() }
        ...
        request.header("User-Agent").shouldNotBeNull().shouldStartWith("karbon/")
      }

      should("send carbone-version: 5 for cloud clients") {
        stub.handler = HttpHandler { ex -> ex.respondJson(200, STATUS_BODY) }
        val karbon = Karbon.cloud("api-key") { baseUrl(stub.baseUrl) }
        either<KarbonError, ApiStatus> { karbon.status() }
      ...
      should("send no Authorization header when the token is null") {
        stub.handler = HttpHandler { ex -> ex.respondJson(200, STATUS_BODY) }
        val karbon = Karbon.onPremise(stub.baseUrl, token = null)
        either<KarbonError, ApiStatus> { karbon.status() }
    ...
    context("templates.upload") {
      should("send a multipart request with name before the file part, legacy response") {
        ...
        val result =
          either<KarbonError, UploadedTemplate> {
      ...
      should("parse a versioned upload response, mapping deployedAt=0 to null") {
        ...
        val result =
          either<KarbonError, UploadedTemplate> {
            karbon.templates.upload(TemplateSource.bytes("x".toByteArray(), "t.docx"))
          }
        val uploaded = result.shouldBeInstanceOf<Either.Right<UploadedTemplate>>().value
    ...
    context("renders.render") {
      should("POST /render/{id}?download=true, send the JSON body and return the file") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result =
          either<KarbonError, RenderedDocument> {
      ...
      should("upload then retry once when the template is unknown, hash-first flow") {
        ...
        val result =
          either<KarbonError, RenderedDocument> {
            karbon.renders.render(TemplateSource.bytes(content), RenderData.Empty)
          }
      ...
      should("not retry a 404 for a remote TemplateSource.id, raising TemplateNotFound") {
        ...
        val result =
          either<KarbonError, RenderedDocument> {
            karbon.renders.render(TemplateSource.id("missing"), RenderData.Empty)
          }
        val error = result.shouldBeInstanceOf<Either.Left<KarbonError>>().value
        val notFound = error.shouldBeInstanceOf<KarbonError.TemplateNotFound>()
      ...
      should("map a 401 to Unauthorized with the raw message") {
        ...
        val result =
          either<KarbonError, RenderedDocument> {
            karbon.renders.render(TemplateSource.id("abc"), RenderData.Empty)
          }
        val error = result.shouldBeInstanceOf<Either.Left<KarbonError>>().value
        val unauthorized = error.shouldBeInstanceOf<KarbonError.Unauthorized>()
    ...
    context("renders.start / download") {
      should("start a render returning a RenderId, with download=false") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result =
          either<KarbonError, RenderId> {
            karbon.renders.start(TemplateSource.id("xyz"), RenderData.Empty)
          }
      ...
      should("download a render result via GET /render/{id}") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result =
          either<KarbonError, RenderedDocument> { karbon.renders.download(RenderId("r1.pdf")) }
        val document = result.shouldBeInstanceOf<Either.Right<RenderedDocument>>().value
        document.content shouldBe pdfBytes
        val request = stub.requests.single()
      ...
      should("map a 404 on download to RenderNotFound") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result =
          either<KarbonError, RenderedDocument> { karbon.renders.download(RenderId("gone.pdf")) }
        result
    ...
    should("send carbone-webhook-url and carbone-webhook-header-<name> for startAsync") {
      ...
      val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
      val result =
        either<KarbonError, AsyncRenderAccepted> {
          karbon.renders.startAsync(
    ...
    should("convert via POST /render/template with a base64 template field and no data key") {
      ...
      val result =
        either<KarbonError, RenderedDocument> {
          karbon.renders.convert(TemplateSource.bytes(doc), OutputFormat.PDF)
        }
      result.shouldBeInstanceOf<Either.Right<RenderedDocument>>()
      val request = stub.requests.single()
    ...
    context("templates") {
      should("delete a template via DELETE /template/{id}") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result = either<KarbonError, Unit> { karbon.templates.delete(TemplateId("t1")) }
      ...
      should("download a template file, exposing its filename") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result =
          either<KarbonError, TemplateFile> { karbon.templates.download(TemplateId("t1")) }
      ...
      should("list templates, reading hasMore/nextCursor at the root, with limit=100") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result = either<KarbonError, Page<TemplateInfo>> { karbon.templates.list() }
      ...
      should("follow the cursor across pages with listAll") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val result = either<KarbonError, List<TemplateInfo>> { karbon.templates.listAll().toList() }
      ...
      should("map categories() and tags() from [{name}] arrays") {
        ...
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val categories = either<KarbonError, List<String>> { karbon.templates.categories() }
        val tags = either<KarbonError, List<String>> { karbon.templates.tags() }
    ...
    should("map a connection failure to KarbonError.Transport") {
      val karbon = Karbon.onPremise("http://127.0.0.1:1") { connectTimeout = 1.seconds }
      val result = either<KarbonError, ApiStatus> { karbon.status() }
      result
        .shouldBeInstanceOf<Either.Left<KarbonError>>()
        .value
        .shouldBeInstanceOf<KarbonError.Transport>()
    }
    ...
    context("blocking facade") {
      should("return successfully for a 2xx response") {
        stub.handler = HttpHandler { ex -> ex.respondJson(200, STATUS_BODY) }
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        karbon.blocking().status() shouldBe ApiStatus(true, 200, "OK", "5.8.0")
      }

      should("throw KarbonException wrapping Unauthorized on a 401") {
        stub.handler = HttpHandler { ex ->
          ex.respondJson(401, """{"success":false,"error":"Invalid token"}""")
        }
        val karbon = Karbon.onPremise(stub.baseUrl, token = "tok")
        val exception =
          shouldThrow<KarbonException> {
            karbon.blocking().renders.render(TemplateSource.id("abc"), RenderData.Empty)
          }
        exception.error.shouldBeInstanceOf<KarbonError.Unauthorized>()
      }
    }
  })
