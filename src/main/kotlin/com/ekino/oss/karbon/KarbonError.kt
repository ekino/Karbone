...
/** Every way a Karbon call can fail, as a value. */
public sealed interface KarbonError {
   ...
   * rewritten.
   */
  public sealed interface Api : KarbonError {
  ...
  public data class InvalidRequest(val reason: String) : KarbonError {
    override fun describe(): String = "Invalid request: $reason"
  }

  /** Connection, timeout or I/O failure. */
  public data class Transport(val cause: Throwable) : KarbonError {
    override fun describe(): String =
      "Transport failure: ${cause.message ?: cause::class.simpleName}"
  }

  /** A response body that could not be parsed. */
  public data class Serialization(val cause: Throwable, val body: String?) : KarbonError {
...
/** Thrown by the blocking facade only. Kotlin callers get [KarbonError] through `Raise`. */
public class KarbonException(public val error: KarbonError) :
  RuntimeException(
    error.describe(),
    (error as? KarbonError.Transport)?.cause ?: (error as? KarbonError.Serialization)?.cause,
  )
