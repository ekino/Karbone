 ...
 */
package com.ekino.oss.karbon.internal.http

import com.ekino.oss.karbon.KarbonConfig
import com.ekino.oss.karbon.RetryPolicy
 ...
 * the retry policy.
 */
internal class JdkHttpTransport(private val config: KarbonConfig, client: HttpClient? = null) :
  HttpTransport {
    ...
    val publisher =
      when (val body = spec.body) {
        ...
        is HttpBody.Multipart -> {
          val boundary = "----karbon-${UUID.randomUUID()}"
